...
{
    /// <summary>
    /// 設定の読み込み・保存と、読み込んだ設定を UI に適用する責務を持つコントローラ。
    /// <para>
    /// - ファイル入出力は内部で SettingsManager に委譲する。
    /// - UI への適用（ウィンドウサイズやサイドバー表示など）は呼び出し元が UI スレッドであることを想定している。
    ...
    internal class SettingsController
    {
        ...
        /// <param name="presenter">Presenter（表示モード復元等に使用）</param>
        ...
        /// <summary>
        /// 設定を読み込み、関連オブジェクトと UI に適用します。
        /// <para>
        /// - 呼び出しは基本的に UI スレッド上で行ってください（ウィンドウサイズや GridLength の操作が行われるため）。
        ...
        public void LoadAndApply()
        {
            ...
            // ウィンドウサイズと状態を復元。WindowState は最大化などを表す符号化値。
            // 注意: 実際の表示スケーリングやモニタ構成により期待どおりにならない場合があるため
            // 必要に応じて範囲チェックや補正を検討してください。
        ...
        /// </summary>
        /// <returns>保存タスク。必要なら呼び出し元で await できます。</returns>
        ...
        /// <summary>
        /// 現在の UI/Presenter の状態を元に AppSettings インスタンスを作成します。
        /// </summary>

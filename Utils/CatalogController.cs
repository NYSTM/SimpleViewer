...
{
    /// <summary>
    /// カタログ表示（サムネイル一覧）の生成と表示制御を担当するコントローラクラス。
    /// - サムネイル生成は非同期で実行し、UI 更新は必ず Dispatcher 経由で行う。
    /// - 表示中の生成処理は CancellationToken によりキャンセル可能とし、不要な作業を中断する。
    /// - MainWindow の UI 要素への直接依存は最低限に留め、テストしやすくする。
    /// </summary>
    public class CatalogController : IDisposable
    {
        ...
        /// - 既存の生成処理があればキャンセルしてから開始します。
        /// - サムネイル生成はキャンセル可能で、失敗は無視して次へ進みます（非致命）。
        ...
        public async Task ShowAsync()
        {
            ...
            // カタログ UI を表示してパネルを初期化する（UI スレッドで実行）
            _dispatcher.Invoke(() =>
            {
            ...
            for (int i = 0; i <= max; i++)
            {
                ...
                if (thumb != null)
                {
                    ...
                    _dispatcher.Invoke(() => _catalogPanel.Children.Add(item));
                }

                // 大量のページがある場合に UI スレッドをスターベーションさせないため適宜 Yield
                if (i % 10 == 0) await Task.Yield();
            }
        }

        /// <summary>
        /// カタログを閉じる。生成中のタスクはキャンセルし、UI を隠す。
        /// </summary>
        public void Close()
        {
            CancelAndDispose();
        ...
        /// クリック時には該当ページへジャンプし、カタログを閉じてウィンドウにフォーカスを戻します。
        /// </summary>
